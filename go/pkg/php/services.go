...
func (s *baseService) Logs(follow bool) (io.ReadCloser, error) { // Result boundary
	if s.logPath == "" {
		return nil, phpFailure("no log file available for %s", s.name)
	}

	m := getMedium()
	file, err := m.Open(s.logPath)
	if err != nil {
		return nil, phpWrapAction(err, "open", "log file")
	}
	...
	if !ok {
		...
		return nil, phpFailure("log file is not a regular file")
	}
	return newTailReader(osFile), nil
}

func (s *baseService) startProcess(ctx context.Context, cmdName string, args []string, env []string) error { // Result boundary
	s.mu.Lock()
	defer s.mu.Unlock()

	if s.running {
		return phpFailure("%s is already running", s.name)
	}
	...
	if !ok {
		...
		return phpFailure("log file is not a regular file")
	}
	s.logFile = logFile
	...
	if err := s.cmd.Start(); err != nil {
		if closeErr := logFile.Close(); closeErr != nil {
			err = phpFailure("%v; close log file: %v", err, closeErr)
		}
		s.lastError = err
		return phpWrapAction(err, "start", s.name)
	}
...
func (s *baseService) stopProcess() error { // Result boundary
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *FrankenPHPService) Start(ctx context.Context) error { // Result boundary
	args := []string{
...
func (s *FrankenPHPService) Stop() error { // Result boundary
...
func (s *ViteService) Start(ctx context.Context) error { // Result boundary
	var cmdName string
	var args []string
...
func (s *ViteService) Stop() error { // Result boundary
	return s.stopProcess()
}
...
func (s *HorizonService) Start(ctx context.Context) error { // Result boundary
	return s.startProcess(ctx, "php", []string{"artisan", "horizon"}, nil)
}

// Stop terminates Horizon using its terminate command.
func (s *HorizonService) Stop() error { // Result boundary
...
func (s *ReverbService) Start(ctx context.Context) error { // Result boundary
	args := []string{
...
func (s *ReverbService) Stop() error { // Result boundary
	return s.stopProcess()
}
...
func (s *RedisService) Start(ctx context.Context) error { // Result boundary
	args := []string{
...
func (s *RedisService) Stop() error { // Result boundary
...
func (t *tailReader) Read(p []byte) (n int, err error) { // Result boundary
	t.mu.RLock()
	if t.closed {
		t.mu.RUnlock()
		return 0, io.EOF
...
func (t *tailReader) Close() error { // Result boundary
	t.mu.Lock()
	t.closed = true
	t.mu.Unlock()
	return t.file.Close()
