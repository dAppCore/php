<ul class="space-y-1">
 ...
 <hr class="flex-1 border-line-1 " />
 <span class="text-xs text-fg-3 dark:text-fg-2 uppercase tracking-wider">{{ $item['label'] }}</span>
 <hr class="flex-1 border-line-1 " />
 </div>
 @else
 ...
 type="button"
 @click="open = !open"
 class="w-full flex items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider {{ $groupColor }} hover:bg-ink-3 rounded-md transition-colors"
 >
 ...
 <ul x-show="open" x-collapse class="mt-1 ml-2 pl-2 border-l border-line-1 space-y-1">
 @foreach($item['children'] ?? [] as $child)
 @if(!empty($child['separator']))
 <li class="py-1"><hr class="border-line-1 " /></li>
 ...
 'cyan' => 'text-cyan-500',
 'pink' => 'text-pink-500',
 default => 'text-fg-3 dark:text-fg-2',
 };
 @endphp
 ...
 <span class="ml-auto text-xs px-1.5 py-0.5 rounded-full bg-ink-1 text-fg-1 ">
